...
* **Обработка команд**:
  ...
  * `/events [город]` — получение информации о предстоящих событиях в указанном городе (интеграция с TimePad API)
...
5. **Интеграция с внешними API** (OpenWeatherMap, NewsAPI, TimePad)
6. **Планировщик задач** APScheduler для регулярного обновления данных и отправки подписных уведомлений.
...
* Получить события в Москве:

  ```
  /events Москва
  ```

* Получить события по умолчанию (без указания города):
...
- **OpenWeatherMap** — для получения прогноза погоды
- **NewsAPI** — для получения новостей
- **TimePad API** — для получения информации о событиях (необходим API-ключ, указывается в `EVENTS_API_KEY`)
